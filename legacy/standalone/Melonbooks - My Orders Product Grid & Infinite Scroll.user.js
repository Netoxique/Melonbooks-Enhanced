// ==UserScript==
// @name         Melonbooks - My Orders Product Grid & Infinite Scroll
// @namespace    https://www.melonbooks.co.jp/
// @version      1.2.2
// @description  Expand Melonbooks order history to the full window width, add persistent grid controls, default the order range to 1 year, and add infinite scroll.
// @author       ChatGPT
...
(function () {
  ...
  class ScriptConfig {
    ...
    // Default number of product columns per order.
    static columnCount = 5;
    static minColumnCount = 2;
    static maxColumnCount = 12;
    static columnCountStorageKey = 'melonbooksOrdersColumnCount';

    // Default spacing between product cards in pixels.
    static productSpacingPx = 0;
    static minProductSpacingPx = 0;
    static maxProductSpacingPx = 100;
    static productSpacingStorageKey = 'melonbooksOrdersProductSpacingPx';

    static fullWidthHorizontalPaddingPx = 12;

    // Change this value to control the vertical spacing inside product info rows.
    static productInfoRowSpacingPx = 1;

    // Use transparent/inherit values so Dark Reader and site themes can control colors.
    static productCardBackground = 'transparent';
    static productCardBorderColor = 'rgba(128, 128, 128, 0.35)';

    // Melonbooks order-period values:
    // 1 = past 3 months, 2 = past 6 months, 3 = past 1 year, 999 = all orders.
    static defaultSearchSelectValue = '3';

    // If true, the script submits the search form once per tab so the actual
    // loaded orders use the default period.
    static autoSubmitDefaultSearchPeriod = true;

    // Infinite scroll behavior.
    static infiniteScrollEnabled = true;
    static infiniteScrollRootMarginPx = 1200;
    static hideOriginalPagination = false;
  }
  ...
  const css = `
    ...
    /* Remove the site's fixed-width order-history constraints. */
    ...
    /* Product card: thumbnail on top, info underneath. */
    ...
    /* Keep the whole order block usable when the site lays order metadata beside products. */
    div.history-detail {
  ...
  function injectStyle() {
    ...
    style.type = 'text/css';
    style.id = 'melonbooks-orders-grid-style';
    style.textContent = css;

    const appendStyle = () => {
      const target = document.head || document.documentElement;

      if (target && !document.getElementById(style.id)) {
  ...
  function clampColumnCount(value) {
    ...
    return Math.min(
      ScriptConfig.maxColumnCount,
      Math.max(ScriptConfig.minColumnCount, parsed)
    );
  }
  ...
  function clampProductSpacing(value) {
    ...
    return Math.min(
      ScriptConfig.maxProductSpacingPx,
      Math.max(ScriptConfig.minProductSpacingPx, parsed)
    );
  }
  ...
  function loadStoredColumnCount() {
    try {
      const storedValue = window.localStorage.getItem(
        ScriptConfig.columnCountStorageKey
      );

      return storedValue === null
        ? ScriptConfig.columnCount
        : clampColumnCount(storedValue);
    } catch (error) {
      console.warn(
        '[Melonbooks Orders Grid] Could not read the saved column count:',
        error
      );
  ...
  function loadStoredProductSpacing() {
    try {
      const storedValue = window.localStorage.getItem(
        ScriptConfig.productSpacingStorageKey
      );

      return storedValue === null
        ? ScriptConfig.productSpacingPx
        : clampProductSpacing(storedValue);
    } catch (error) {
      console.warn(
        '[Melonbooks Orders Grid] Could not read the saved product spacing:',
        error
      );
  ...
  function saveColumnCount(value) {
    try {
      ...
      console.warn(
        '[Melonbooks Orders Grid] Could not save the column count:',
        error
      );
    }
  }
  ...
  function saveProductSpacing(value) {
    try {
      ...
      console.warn(
        '[Melonbooks Orders Grid] Could not save the product spacing:',
        error
      );
    }
  }
  ...
  function updateColumnCountControl() {
    const decreaseButton = document.getElementById(
      'mb-orders-column-decrease'
    );
    const increaseButton = document.getElementById(
      'mb-orders-column-increase'
    );
    const valueOutput = document.getElementById(
      'mb-orders-column-value'
    );

    if (valueOutput) {
      valueOutput.value = String(activeColumnCount);
      valueOutput.textContent = String(activeColumnCount);
      valueOutput.setAttribute(
        'aria-label',
        `${activeColumnCount} products per row`
      );
    }

    if (decreaseButton) {
      decreaseButton.disabled =
        activeColumnCount <= ScriptConfig.minColumnCount;

      decreaseButton.title =
        `Decrease products per row. Minimum: ` +
        `${ScriptConfig.minColumnCount}.`;
    }

    if (increaseButton) {
      increaseButton.disabled =
        activeColumnCount >= ScriptConfig.maxColumnCount;

      increaseButton.title =
        `Increase products per row. Maximum: ` +
        `${ScriptConfig.maxColumnCount}.`;
    }
  }
  ...
  function updateProductSpacingControl() {
    const decreaseButton = document.getElementById(
      'mb-orders-spacing-decrease'
    );
    const increaseButton = document.getElementById(
      'mb-orders-spacing-increase'
    );
    const valueOutput = document.getElementById(
      'mb-orders-spacing-value'
    );
    ...
    if (valueOutput) {
      ...
      valueOutput.setAttribute(
        'aria-label',
        `${activeProductSpacingPx} pixels between products`
      );
    }

    if (decreaseButton) {
      decreaseButton.disabled =
        activeProductSpacingPx <=
        ScriptConfig.minProductSpacingPx;

      decreaseButton.title =
        `Decrease product spacing. Minimum: ` +
        `${ScriptConfig.minProductSpacingPx}px.`;
    }
    ...
    if (increaseButton) {
      ...
      increaseButton.title =
        `Increase product spacing. Maximum: ` +
        `${ScriptConfig.maxProductSpacingPx}px.`;
    }
  }
  ...
  function applyColumnCount(value, persist = true) {
    ...
    if (document.documentElement) {
      document.documentElement.style.setProperty(
        '--mb-orders-column-count',
        String(activeColumnCount)
      );
    }
  ...
  function createProductsPerRowControl() {
    ...
    const decreaseButton = createSettingButton({
      ...
      onClick: () => {
        applyColumnCount(activeColumnCount - 1);
      }
    });
  ...
  function createProductSpacingControl() {
    ...
    const decreaseButton = createSettingButton({
      ...
      onClick: () => {
        applyProductSpacing(activeProductSpacingPx - 1);
      }
    });
  ...
  function initGridControls() {
    ...
    if (
      !myPage ||
      document.querySelector('.mb-orders-grid-controls')
    ) {
    ...
    const productsPerRowControl =
      createProductsPerRowControl();

    const spacingControl =
      createProductSpacingControl();

    controls.append(
      productsPerRowControl,
      spacingControl
    );

    const pageHeader = myPage.querySelector(
      ':scope > .page-header'
    );
  ...
  function getSearchSelect(root = document) {
    return root.querySelector(
      'form#form1 select[name="search_select"]'
    );
  }
  ...
  function setDefaultSearchPeriod() {
    ...
    const desiredValue = String(
      ScriptConfig.defaultSearchSelectValue
    );
    ...
    if (
      ...
      select.querySelector(
        `option[value="${CSS.escape(desiredValue)}"]`
      )
    ) {
      select.value = desiredValue;
    ...
    if (!ScriptConfig.autoSubmitDefaultSearchPeriod) {
      return;
    }
  ...
  function getOrderListContainer(root = document) {
    const directContainers = Array.from(
      root.querySelectorAll('.my-page > div')
    );

    const directMatch = directContainers.find(
      (element) => {
    ...
    const firstOrder = root.querySelector(
      '.history-detail'
    );

    return firstOrder
      ? firstOrder.parentElement
      : null;
  }
  ...
  function getCurrentPageNumber(root = document) {
    ...
    const fromTitle = extractPageNumber(
      current &&
    ...
    const fromText = extractPageNumber(
      current &&
        current.textContent
    );
    ...
    const pageInput =
      form &&
      form.querySelector('[name="pageno"]');

    const fromInput = extractPageNumber(
      pageInput &&
        pageInput.value
    );
  ...
  function getNextPageNumber(root = document) {
    ...
    const currentPage =
      getCurrentPageNumber(root);

    const numericLinks = Array.from(
  ...
  function getOrderKey(orderElement) {
    const rows = Array.from(
      orderElement.querySelectorAll(
        '.history-detail__table tr'
      )
    );
  ...
  function initInfiniteScroll() {
    if (
      !ScriptConfig.infiniteScrollEnabled
    ) {
      return;
    }

    const orderListContainer =
      getOrderListContainer();
    ...
    pagination.parentElement.insertBefore(
      sentinel,
      pagination
    );
    ...
    if (
      ScriptConfig.hideOriginalPagination
    ) {
      document.body.classList.add(
        'mb-orders-hide-pagination'
      );
    }
    ...
    async function loadNextPage() {
      if (
        ...
        isFinished ||
        !nextPageNumber ||
        loadedPages.has(nextPageNumber)
      ) {
        return;
      }
      ...
      try {
        ...
        if (!request) {
          throw new Error(
            'Could not build the next-page request.'
          );
        }
        ...
        for (
          ...
          orderListContainer.appendChild(
            document.importNode(
              incomingOrder,
              true
            )
        ...
        const parsedNextPageNumber =
          getNextPageNumber(
            nextDocument
          );

        if (
          parsedNextPageNumber &&
          !loadedPages.has(
            parsedNextPageNumber
          )
        ) {
          ...
          setStatus(
            'All order pages loaded.'
          );
        }
      } catch (error) {
        console.error(
          '[Melonbooks Orders Grid] Infinite scroll failed:',
          error
        );

        setStatus(
          'Could not load the next order page. Use the page numbers below as fallback.'
        );
      } finally {
        isLoading = false;
      }
      ...
      const observer =
        new IntersectionObserver(
          ...
          },
          {
            root: null,
            rootMargin:
              `${ScriptConfig.infiniteScrollRootMarginPx}px 0px`,
            threshold: 0
          }
        );
